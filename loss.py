...
def train_step(generator,discriminator, image_list, mask_image_list, mask_list):  # image为原图 0 - 1.0
    with tf.GradientTape() as tape,tf.GradientTape() as dis_tape:
        ...
        pyramid_feats,gen_image_list = generator([mask_image_list, 1.0 - mask_list])
        ...
        dis_loss = (calv(dis_real_feat,1) + calv(dis_fake_feat,0))/2.
        
        for feat in pyramid_feats:
            pyramid_loss += l1_loss(feat,tf.image.resize(images,feat.shape[1:3],"bilinear"))
        
        hole_loss = l1_loss(gen_image_list*(1-mask_list), image_list*(1-mask_list))/tf.reduce_mean(1-mask_list)
        valid_loss = l1_loss(gen_image_list*mask_list, image_list*mask_list)/tf.reduce_mean(mask_list)
        adversarial_loss = calv(dis_fake_feat,1)
